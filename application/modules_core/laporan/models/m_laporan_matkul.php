	...
	public function getPeserta($kode,$grup) {
		$sql = "SELECT k.id_kelasb, u.nim, u.nama_lengkap, j.id_jawaban FROM
				(SELECT id_kelasb FROM ec_kelas_buka kb WHERE kb.kode = '$kode' AND kb.grup = '$grup'
					AND kb.thn_ajaran = (SELECT MAX(thn_ajaran) FROM ec_kelas_buka WHERE thn_ajaran = (SELECT MAX(thn_ajaran) as thn_ajaran FROM ec_kelas_buka))
					AND kb.semester = (SELECT MAX(semester) FROM ec_kelas_buka WHERE thn_ajaran = (SELECT MAX(thn_ajaran) as thn_ajaran FROM ec_kelas_buka))) k

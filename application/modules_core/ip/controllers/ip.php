...
class Ip extends MX_Controller
{
	...
	public function __construct()
	{
		...
		if (!isset($this->session->userdata['is_admin']) || !$this->session->userdata['is_admin'] ) {			
			if ($this->session->userdata['is_kepala_unit'] != true ) {
	...
	function detail_dosen_perprodi_pdf($id_unit = NULL, $id_paket = NULL){
		...
		if (isset($this->session->userdata['unit_kepala_unit'])) {
			#if kepala unit check
			if ($id_unit != $this->session->userdata['unit_kepala_unit']) {
				redirect('ip/ip');			
			}
		}

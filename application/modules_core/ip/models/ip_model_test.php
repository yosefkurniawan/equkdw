...
class ip_model_test extends CI_Model
{
	...
	{	
		$sql_list_kd_diajar = "
			SELECT nik, nama_dsn, kode, nama_mtk, grup, mykey, semester, thn_ajaran as th_ajaran FROM kelas_all 
			WHERE thn_ajaran = '$th_ajaran' AND semester = '$semester' AND nik = '$nik' AND eva_status = '1'";
		...
		$sql_list_mhs_diajar = "
			SELECT
			  ...
			  k.th_ajaran AS th_ajaran,
			  n.nilai AS nilai,
			  sum((case when (k.absen = 'H') then 1 else 0 end)) AS count_hadir,
			  sum((case when (k.absen = 'A') then 1 else 0 end)) AS count_absen,
			  ((sum((case when (k.absen = 'H') then 1 else 0 end)) * 100) / (sum((case when (k.absen = 'H') then 1 else 0 end)) + sum((case when (k.absen = 'A') then 1 else 0 end)))) AS persen_hadir,
			  list_kd_diajar.nik
			FROM o3_raw_kehadiran k
			INNER JOIN o3_raw_nilai n ON n.nim = k.nim AND n.kode = k.kode AND n.grup = k.grup AND n.prodi = k.prodi AND n.semester = k.semester AND k.th_ajaran = k.th_ajaran
			INNER JOIN ($sql_list_kd_diajar) AS list_kd_diajar ON list_kd_diajar.kode = k.kode
			WHERE k.semester = '$semester' AND k.th_ajaran = '$th_ajaran'
			GROUP BY nim, kode, grup, prodi, semester, th_ajaran, nilai";
		
		$sql_select_o4 = "SELECT DISTINCT * FROM o4_nilaimasuk";
		...
		$sql = "SELECT matkul.nik as nik_baru, matkul.nama_dsn, matkul.kode, matkul.nama_mtk, matkul.grup, matkul.mykey,
			...
			o1.rencana,
			o1.tot_hadir,
			sum((case when (list_mhs_diajar.nilai = 'A' AND list_mhs_diajar.persen_hadir > 50) then 1 else 0 end)) AS tot_nilai_A,
			sum((case when (list_mhs_diajar.nilai = 'A-' AND list_mhs_diajar.persen_hadir > 50) then 1 else 0 end)) AS tot_nilai_Amin,
			sum((case when (list_mhs_diajar.nilai = 'B+' AND list_mhs_diajar.persen_hadir > 50) then 1 else 0 end)) AS tot_nilai_Bplus,
			sum((case when (list_mhs_diajar.nilai = 'B' AND list_mhs_diajar.persen_hadir > 50) then 1 else 0 end)) AS tot_nilai_B,
			sum((case when (list_mhs_diajar.nilai = 'B-' AND list_mhs_diajar.persen_hadir > 50) then 1 else 0 end)) AS tot_nilai_Bmin,
			sum((case when (list_mhs_diajar.nilai = 'C+' AND list_mhs_diajar.persen_hadir > 50) then 1 else 0 end)) AS tot_nilai_Cplus,
			sum((case when (list_mhs_diajar.nilai = 'C' AND list_mhs_diajar.persen_hadir > 50) then 1 else 0 end)) AS tot_nilai_C,
			sum((case when (list_mhs_diajar.nilai = 'D' AND list_mhs_diajar.persen_hadir > 50) then 1 else 0 end)) AS tot_nilai_D,
			sum((case when (list_mhs_diajar.nilai = 'E' AND list_mhs_diajar.persen_hadir > 50) then 1 else 0 end)) AS tot_nilai_E,
			sum((case when (list_mhs_diajar.nilai = 'F' AND list_mhs_diajar.persen_hadir > 50) then 1 else 0 end)) AS tot_nilai_F,
			o3.tot_lulus,
			o3.tot_mhs
			FROM
			($sql_list_kd_diajar) matkul
			...
			LEFT JOIN ($sql_select_o4) o4 ON o4.mykey = matkul.mykey
			LEFT JOIN o5_eclass o5 ON o5.mykey = matkul.mykey AND o5.nik = matkul.nik
			LEFT JOIN ($sql_list_mhs_diajar) AS list_mhs_diajar ON list_mhs_diajar.nik = matkul.nik AND list_mhs_diajar.kode = matkul.kode AND list_mhs_diajar.grup = matkul.grup AND list_mhs_diajar.th_ajaran = matkul.th_ajaran AND list_mhs_diajar.semester = matkul.semester
			GROUP BY nik_baru, matkul.nama_dsn, matkul.kode, matkul.nama_mtk, matkul.grup, matkul.mykey,
			o1.persen_hadir,o2.baik,o3.persen_lulus,o4.flag_tepat,
			o5.silabus,o5.materi,o5.tugas,o5.nilai,o5.eclass,
			o4.tgl_masuk, 
			o1.rencana,
...
/*
		$sql_cek_o4 = "
			SELECT DISTINCT * FROM o4_nilaimasuk 
			WHERE mykey = 'SI2036AGASAL2014/2015'";

		$kd_diajar = $this->db->query($sql_list_kd_diajar);		
		echo "MATAKULIAH DIAJAR (".count($kd_diajar->result())."):";
		echo '<pre>'; print_r($kd_diajar->result()); 
		echo "<br/>";
*/

/*
		$mhs_diajar = $this->db->query($sql_list_mhs_diajar);
		echo "MAHASISWA DIAJAR (".count($mhs_diajar->result())."):";		
		echo '<pre>'; print_r($mhs_diajar->result()); 
		echo "<br/>";
*/

	/*
	$query = $this->db->query($sql);
		echo "HASIL AKHIR (".count($query->result())."):";
		echo '<pre>'; print_r($query->result()); 
		echo "<br/>";
		
		$query_cek_o4 = $this->db->query($sql_cek_o4);
		echo "Cek o4 (".count($query_cek_o4->result())."):";
		echo '<pre>'; print_r($query_cek_o4->result()); 
		echo "<br/>";
...
/*
		...
		echo "SQL LIST KODE:";		
		echo '<pre>'; print_r($sql_list_kd_diajar); 
		echo "<br/>";
		
		echo "SQL LIST MHS:";		
		echo '<pre>'; print_r($sql_list_mhs_diajar); 
		echo "<br/>";
*/
		/*

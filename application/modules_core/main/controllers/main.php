...
/* 		print_r($this->session->userdata['unit_kepala_unit']);die; */
	}
		...
		if(!empty($_POST)){
			...
			if ($result_login_SSAT['result']) {
				...
				if ($is_kepala_unit) 
				{
					...
					# get unit
					$unit = $this->m_login->get_unit_of_kepala_unit($SSAT_id_user);
					$this->session->set_userdata('unit_kepala_unit', $unit->id_unit);
